...
export function QuotationDialog({ clients, projects, quotation }: { clients: ClientOption[]; projects: ProjectOption[]; quotation?: QuotationRow }) {
  ...
  const [items, setItems] = useState<Array<{ description: string; qty: string; unit: string; unitPrice: string; unitCost: string }>>(
    quotation?.items?.length ? quotation.items.map((item) => ({ description: item.description, qty: String(item.qty), unit: item.unit || "Unit", unitPrice: String(item.unitPrice), unitCost: String((item as any).unitCost ?? 0) })) : [{ description: "", qty: "1", unit: "Unit", unitPrice: "", unitCost: "" }],
  );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-6">
          ...
          <div className="rounded-xl border p-4 space-y-4"><div className="flex items-center justify-between"><h3 className="font-semibold">Item Quotation</h3><Button type="button" variant="outline" size="sm" onClick={() => setItems([...items, { description: "", qty: "1", unit: "Unit", unitPrice: "", unitCost: "" }])}>Tambah Item</Button></div><div className="grid gap-3">{items.map((item, index) => (<div key={index} className="grid gap-3 rounded-lg border p-3 md:grid-cols-12"><div className="md:col-span-4"><Label>Nama Barang/Jasa + Spesifikasi</Label><Textarea rows={4} value={item.description} onChange={(e) => setItems(items.map((row, i) => i === index ? { ...row, description: e.target.value } : row))} placeholder="Contoh: SAMSUNG 75 inch 4K Smart TV\nSpesifikasi: ..." /></div><div className="md:col-span-1"><Label>Qty</Label><Input type="number" value={item.qty} onChange={(e) => setItems(items.map((row, i) => i === index ? { ...row, qty: e.target.value } : row))} /></div><div className="md:col-span-1"><Label>Unit</Label><Input value={item.unit} onChange={(e) => setItems(items.map((row, i) => i === index ? { ...row, unit: e.target.value } : row))} placeholder="Unit / Pcs / Set" /></div><div className="md:col-span-2"><Label>Harga Jual</Label><RupiahInput value={item.unitPrice} onChange={(value) => setItems(items.map((row, i) => i === index ? { ...row, unitPrice: value } : row))} placeholder="5000000" /></div><div className="md:col-span-2"><Label>Modal (Cost)</Label><RupiahInput value={item.unitCost} onChange={(value) => setItems(items.map((row, i) => i === index ? { ...row, unitCost: value } : row))} placeholder="4000000" /></div><div className="md:col-span-2"><Label>Margin Item</Label><div className={cn("text-sm font-medium p-2 rounded border", ((parseMoneyInput(item.unitPrice) - parseMoneyInput(item.unitCost)) * Number(item.qty || 0)) >= 0 ? "bg-green-50 border-green-200 text-green-700" : "bg-red-50 border-red-200 text-red-700")}><span className="block">{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format((parseMoneyInput(item.unitPrice) - parseMoneyInput(item.unitCost)) * Number(item.qty || 0))}</span><span className="text-xs ml-1 opacity-70">{(((parseMoneyInput(item.unitPrice) - parseMoneyInput(item.unitCost)) / (parseMoneyInput(item.unitPrice) || 1)) * 100).toFixed(1)}%</span></div></div><div className="md:col-span-12 flex justify-end"><Button type="button" variant="ghost" size="sm" className="text-destructive" onClick={() => setItems(items.length === 1 ? items : items.filter((_, i) => i !== index))}>Hapus Item</Button></div></div>))}</div><div className="rounded-lg border bg-muted/30 p-4 space-y-2 text-sm"><div className="flex justify-between"><span>Subtotal (Penjualan)</span><span>{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(subtotal)}</span></div><div className="flex justify-between text-orange-600 font-medium"><span>Total Modal (Cost)</span><span>{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(computedItems.reduce((sum, item) => sum + (parseMoneyInput(item.unitCost ?? 0) * Number(item.qty || 0)), 0))}</span></div><div className={cn("flex justify-between font-semibold", (subtotal - computedItems.reduce((sum, item) => sum + (parseMoneyInput(item.unitCost ?? 0) * Number(item.qty || 0)), 0)) >= 0 ? "text-green-600" : "text-red-600")}><span>Total Margin</span><span>{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(subtotal - computedItems.reduce((sum, item) => sum + (parseMoneyInput(item.unitCost ?? 0) * Number(item.qty || 0)), 0))} ({((subtotal - computedItems.reduce((sum, item) => sum + (parseMoneyInput(item.unitCost ?? 0) * Number(item.qty || 0)), 0)) / (subtotal || 1) * 100).toFixed(1)}%)</span></div><div className="flex justify-between"><span>PPN ({normalizedTaxPercent}%)</span><span>{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(taxAmount)}</span></div><div className="flex justify-between font-semibold text-base border-t pt-2 mt-2"><span>Total (Termasuk PPN)</span><span>{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(total)}</span></div></div></div>
